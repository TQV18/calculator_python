...
class LOGIC:
    def __init__(self, gui, expression):
        ...
        self.gui = gui
        self.calc = calc_engine(self.expression)
    def process(self, expression): #is used to analyze %, process_percentage
        temp = "" 
        ans = ""
                ...
                if temp:
                    if temp[-1] == '%':
                        ...
                        if float(temp) == int(temp):
                            temp = int(temp)
                    temp = str(temp)
                    while (temp[0] == '0' and len(temp)>1 and temp[1] != '.'):
                        temp = temp[1::]
                    ans += temp
                    temp = ""
                ans += ch
    ...
    def calculate(self):
        ...
        try:
            self.calc = calc_engine(data)
            res = self.calc.evaluate()
...
# ----------------------- #
class calc_engine: #shunting yard
    def __init__(self, expression):
        self.expression = expression
    def tokenize(self): #split digit and op distinguishably
        tk = []
        op = "+-/*"
        temp = ""
        ...
        for i in range(len(tk)):
            if tk[i] == '-' and (i == 0 or tk[i-1] in "+-*/"):
                new_tk.append('0')
            new_tk.append(tk[i])
        tk = new_tk
        return tk
    def parse(self): #convert infix to postfix
        ...
        for i in tokens:
            if (i.replace('.', '', 1).isdigit()):
                ...
                while (len(op_list)!=0 and prio[i] <= prio[op_list[-1]]):
                    digit_list.append(op_list[-1])
                    op_list.pop()
                op_list.append(i)
        while (len(op_list) != 0):
            digit_list.append(op_list[-1])
